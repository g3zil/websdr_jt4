...
# get metadata from previous jt4 decode using awk add in the current time, noise and level measurements and mode and save as csv
awk -F"," -v OFS="," -v date="${DECODE_CAPTURE_DATE}" -v noise="${RMS_NOISE}" -v signal="${RMS_SIGNAL}" -v mode="${MODE}"  '{print date,$2,$3,$4,$12,$13,$14,$15,noise,signal,mode}' <${BASE_DIR}/spots_azi.csv >${BASE_DIR}/signal_noise.csv
