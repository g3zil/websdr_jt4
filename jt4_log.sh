...
source .venv/bin/activate  # run all the code in the protected environment readme now has note this is a must
...
then
  ...
  then
    ...
    python3 ${BASE_DIR}/azi_calc.py ${DECODE_CAPTURE_DATE} ${RX_GRID} ${RX_ID}                           # calculate lat/lon/azimuths/distance
    echo "Decode data and added variables written to spots_azi.csv file"
    python3  ${BASE_DIR}/jt4_upload.py ${BASE_DIR}/spots_azi.csv                                         # upload to table jt4 database tutorial on wd1 server
  ...
  #  and furthermore, is easily modified for PI4 digimode. The detector is based on code from Daniel Estevez, details in jt4_detect.py
    ...
    python3 ${BASE_DIR}/jt4_detect.py ${DECODE_CAPTURE_DATE} ${BASE_DIR}/11025.wav     # do the processing!
  fi
else
  ...
  if [[ ${WAV_FILE_MINUTE} =~ ^[13579]+$ ]]
     then
       ...
       then
         ...
         python3 ${BASE_DIR}/sn_upload.py ${BASE_DIR}/signal_noise.csv    # uploads to jt4 table on wd1 with mode ident CW together with metadata
       else
